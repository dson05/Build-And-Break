...
static int valid_name(const char *name) {
    ...
    while (*name != '\0') {
        if (!isalpha((unsigned char)*name)) return 0;
        if (++len > 250) return 0;
        name++;
    }
    return 1;
...
static int bank_request(ATM *atm, const char *plain_request, char *plain_response, size_t response_size) {
    ...
    int wlen, dlen, attempts, reply_len;
    ssize_t n;

    if (!rand_bytes(nonce, ATM_NONCE_BYTES)) return 0;
    ...
    for (attempts = 0; attempts < ATM_RECV_ATTEMPTS; attempts++) {
        ...
        if (dlen < header_len || dlen >= (int)sizeof(dec_buf)) continue;
        /* require server tag, the same nonce we just sent, and a separator */
        if (dec_buf[0] != 'A' || dec_buf[1] != ' ' || dec_buf[header_len - 1] != ' ') continue;
        if (memcmp(dec_buf + 2, nonce_hex, 2 * ATM_NONCE_BYTES) != 0) continue;

        reply_len = dlen - header_len;
        if ((size_t)reply_len >= response_size) return 0;
...
ATM* atm_create() {
    ...
    /* recv timeout so a silent or hostile network does not hang the ATM */
    tv.tv_sec = 0;
    tv.tv_usec = 500000;
...
void atm_process_command(ATM *atm, char *command) {
    ...
    if (argc == 0) { printf("Invalid command\n"); return; }

    if (strcmp(argv[0], "begin-session") == 0) {

        /* start a session after card and pin checks */
        char request[512], response[256], card_name[256];
        char card_secret_hex[SECRET_HEX_SIZE], entered_pin[100];
        FILE *card;

        if (atm->logged_in) { printf("A user is already logged in\n"); return; }
        if (argc != 2 || !valid_name(argv[1])) { printf("Usage: begin-session <user-name>\n"); return; }
        ...
        if (card == NULL) { printf("Unable to access %s's card\n", argv[1]); return; }
        if (fgets(card_secret_hex, sizeof(card_secret_hex), card) == NULL) card_secret_hex[0] = '\0';
        fclose(card);
        card_secret_hex[strcspn(card_secret_hex, "\r\n")] = '\0';

        printf("PIN? ");
        fflush(stdout);
        if (fgets(entered_pin, sizeof(entered_pin), stdin) == NULL) { printf("Not authorized\n"); return; }
    ...
    if (strcmp(argv[0], "withdraw") == 0) {
        ...
        char request[300], response[256];
        int amount;

        if (!atm->logged_in) { printf("No user logged in\n"); return; }
        if (argc != 2 || !parse_amount(argv[1], &amount)) { printf("Usage: withdraw <amt>\n"); return; }
    ...
    if (strcmp(argv[0], "balance") == 0) {
        ...
        if (!atm->logged_in) { printf("No user logged in\n"); return; }
        if (argc != 1) { printf("Usage: balance\n"); return; }
