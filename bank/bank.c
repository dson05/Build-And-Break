...
static User *find_user(Bank *bank, const char *name) {
    ...
    for (i = 0; i < bank->num_users; i++) {
        if (strcmp(bank->users[i].name, name) == 0) return &bank->users[i];
    }
    return NULL;
}
...
static int nonce_seen(Bank *bank, const unsigned char *nonce) {
    ...
    for (i = 0; i < BANK_NONCE_RING; i++) {
        if (bank->nonce_used[i] && memcmp(bank->nonces[i], nonce, BANK_NONCE_BYTES) == 0) return 1;
    }
    return 0;
}
...
void bank_process_local_command(Bank *bank, char *command, size_t len) {
    ...
    if (strcmp(argv[0], "deposit") == 0) {
        ...
        if (user == NULL) { printf("No such user\n"); return; }
        if (amount > INT_MAX - user->balance) { printf("Too rich for this program\n"); return; }
        user->balance += amount;
    ...
    if (strcmp(argv[0], "balance") == 0) {
        ...
        User *user;
        if (argc != 2 || !valid_name(argv[1])) { printf("Usage:  balance <user-name>\n"); return; }
        user = find_user(bank, argv[1]);
        if (user == NULL) { printf("No such user\n"); return; }
        printf("$%d\n", user->balance);
        return;
...
static void handle_request(Bank *bank, char *plain, char *reply, size_t reply_size) {
    ...
    if (argc == 2 && strcmp(argv[0], "exists") == 0) {
        if (find_user(bank, argv[1]) != NULL) strcpy(reply, "OK");
        return;
    }
    if (argc == 4 && strcmp(argv[0], "auth") == 0) {
        ...
        user = find_user(bank, argv[1]);
        if (user == NULL) return;
        if (strcmp(user->pin, argv[2]) != 0) return;
        if (!from_hex(argv[3], given, SECRET_SIZE)) return;
        if (memcmp(user->card_secret, given, SECRET_SIZE) != 0) return;
        strcpy(reply, "OK");
        return;
    }
    if (argc == 2 && strcmp(argv[0], "balance") == 0) {
        user = find_user(bank, argv[1]);
        if (user != NULL) snprintf(reply, reply_size, "BAL %d", user->balance);
        return;
    }
    if (argc == 3 && strcmp(argv[0], "withdraw") == 0) {
        user = find_user(bank, argv[1]);
        if (user == NULL || !parse_amount(argv[2], &amount)) return;
        if (amount > user->balance) { strcpy(reply, "NSF"); return; }
        user->balance -= amount;
        strcpy(reply, "OK");
        return;
    }
